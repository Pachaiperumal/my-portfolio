    ...
    <section id="skills" className="py-24 relative overflow-hidden bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-900 via-slate-950 to-slate-950">
      ...
      <div className="container mx-auto px-6 relative z-10 max-w-7xl">
        ...
        >
          ...
          <h2 className="playfair-display-bold text-4xl md:text-5xl lg:text-6xl font-black mb-6 text-white pb-2 relative inline-flex justify-center flex-col items-center">
            Tech Stack
          ...
          <p className="poppins-light mt-8 text-lg text-slate-400 max-w-2xl mx-auto">
        ...
        >
          ...
          {skillCategories.map((category) => (
            <motion.div
              ...
              className={`glass p-8 md:p-10 rounded-3xl border border-white/5 transition-all duration-300 group relative overflow-hidden ${category.borderGlow} hover:shadow-2xl hover:shadow-violet-500/10`}
            >
              ...
              <div className="relative z-10">
                ...
                <h3 className="montserrat-bold text-2xl mb-8 text-white relative inline-block">
                  {category.title}
                  <div className="absolute -bottom-2 left-0 w-12 h-[2px] bg-white/20 group-hover:bg-white/50 group-hover:w-full transition-all duration-300" />
                </h3>
                ...
                <div className="flex flex-wrap gap-3">
                  {category.skills.map((skill) => (
                    <motion.span
                      ...
                      whileTap={{ scale: 0.95 }}
                      className={`flex items-center gap-2 px-4 py-2 bg-slate-900/80 backdrop-blur-md rounded-xl text-sm font-medium border border-white/5 text-slate-300 shadow-lg hover:shadow-xl transition-all cursor-default ${category.pillHoverBg}`}
                    >
                      {skill.icon}
                      {skill.name}
                    </motion.span>
